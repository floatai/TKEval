...
class TokenizedDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        for prompt in prompts:
            token_ids = [int(x) for x in prompt.split(",")]
            if len(token_ids) > self.max_length:
                raise RuntimeError("[WARNING] token length %d larger than max_length!!!! %" % len(token_ids))
            token_ids_pad = token_ids + [self.tokenizer.pad_token_id] * (self.max_length - len(token_ids))
            mask = [1] * len(token_ids) + [0] * (self.max_length - len(token_ids))
        ...
        with torch.no_grad():
            ...
            if "ids_encoder" in batch:
                ...
                if is_wrapped:
                    ...
                    # print("batch end!!! cost: %ds" % (time.time() - start_time))
